      ...
      <div class="col-lg-8">
        <div class="card shadow-sm border-light-subtle rounded-3 h-100 bg-white">
          <div class="card-header bg-white border-bottom py-3 d-flex justify-content-between align-items-center">
            <h6 class="mb-0 fw-bold text-dark"><i class="bi bi-journal-bookmark-fill me-2 text-primary"></i>Catálogo de Asignaturas</h6>
          ...
          <div class="card-body p-0">
            ...
            <div v-if="cargandoTabla" class="text-center py-5 text-muted">
              ...
              <p class="small">Cargando materias...</p>
            </div>
          ...
          <div class="card-body p-4">
            ...
            <form @submit.prevent="guardarAsignatura">
              ...
              <div class="mb-3">
                ...
                <div v-if="modoEdicion" class="form-text small text-warning mt-1">
...
const claveOriginal = ref(null); // Para saber qué clave actualizar en el PUT
...
const guardarAsignatura = async () => {
  ...
  try {
    ...
    if (modoEdicion.value) {
      // Mandamos la claveOriginal a la URL, y el payload en el cuerpo
...
const eliminarAsignatura = async (clave) => {
  ...
  try {
    ...
    if (status === 403) mostrarMensaje('danger', 'No tienes permisos para borrar este registro.', 'bi-shield-lock-fill');
    else if (status === 409 || error.response?.data?.ProtectedError) mostrarMensaje('warning', 'No se puede borrar porque existen reservas asignadas a esta materia.', 'bi-exclamation-triangle-fill');
...
const prepararEdicion = (asignatura) => {
  ...
  claveOriginal.value = asignatura.clave_asignatura; // Guardamos la clave real por si acaso
